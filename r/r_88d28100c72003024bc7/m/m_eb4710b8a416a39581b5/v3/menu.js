...
function show(lang){if(!ui[lang])return;current=lang;const t=ui[lang];document.querySelectorAll('[data-policy]').forEach((el,i)=>el.textContent=policyLabels[lang][i]);document.documentElement.lang=lang;document.title='La Paella · '+t.sample;
...
welcome.hidden=true;menu.hidden=false;const url=new URL(location.href);url.searchParams.set('lang',lang);url.hash='';history.replaceState(null,'',url);window.scrollTo({top:0,behavior:'instant'});document.getElementById('language-back').focus({preventScroll:true});updateCategory();}
document.querySelectorAll('[data-language]').forEach(b=>b.addEventListener('click',()=>show(b.dataset.language)));
document.getElementById('language-back').addEventListener('click',()=>{menu.hidden=true;welcome.hidden=false;document.documentElement.lang='es';document.title='La Paella · Nuestra carta';const url=new URL(location.href);url.searchParams.delete('lang');url.hash='';history.replaceState(null,'',url);window.scrollTo({top:0,behavior:'instant'});document.querySelector('[data-language="'+current+'"]').focus({preventScroll:true});});
const selected=new URLSearchParams(location.search).get('lang');if(ui[selected])show(selected);
